...
summary(lm(a254~interp_254+I(interp_254^2)+I(interp_254^3)+sensor_temp,suna_cols))
...
nitrate$dtpr<-nitrate$dtp
nitrate$nitrate_umL<-nitrate$NSW_15_minute.surfWaterNitrateMean

suna_rn<-merge(suna_red,nitrate[,c("dtpr","nitrate_umL")],by=c("dtpr"),all.x=TRUE)

#MART_wqg<-loadByProduct(dpID="DP1.20093.001",site="MART",check.size=F)
#saveRDS(MART_wqg,"~/gdrive/fdom/SUNA_NEON/MART/MART_Nitrate.rds")
MART_wqg<-readRDS("~/gdrive/fdom/SUNA_NEON/MART/MART_wqg.rds")
...
suna_grab<-merge(suna_cols,date_cast,by="dtpr",all.y=TRUE)
...
pars_sum<-summaryBy(PAR~dtp,pars,FUN=c(mean))


cs2_sum<-summaryBy(.~dtp,compiled_suna2,FUN=c(mean))

temp_par_zoo<-zoo(pars_sum$PAR.mean,pars_sum$dtp)
tempc<-na.approx(temp_par_zoo,xout=cs2_sum$dtp,na.rm=FALSE)
