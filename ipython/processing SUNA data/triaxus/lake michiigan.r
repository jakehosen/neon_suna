...
working<-"~/neon_suna"
site_id<-"Michigan_CSMI_July"
data<-"/Volumes/GoogleDrive/My Drive/SUNA_Data/triaxus/Triaxus_2015_2017_Data/Suna/2015/CSMI July MI"
...
saveRDS(compiled_suna,"/Volumes/GoogleDrive/My Drive/SUNA_Data/triaxus/Triaxus_2015_2017_Data/Suna/2017/michigan_csmi_jul2015_compiled.rds")
...
suna_cols_fl<-suna_cols_f[c(1:22391),]	
suna_cols_fl$a254<-suna_cols_fl$a254+0.03
suna_cols_fl$s275295<-s275295_comp$s275295
suna_cols_fl$s275295z<-abs(((suna_cols_fl$s275295-mean(suna_cols_fl$s275295,na.rm=TRUE))/(sd(suna_cols_fl$s275295,na.rm=TRUE)))/100)+0.6
suna_cols_flm<-melt(subset(suna_cols_fl,date=="2015187"&hour(dtp)>12)[,c("dtp","a254","s275295z")],id.vars="dtp")
suna_cols_flm$variable<-gsub("a254","Absorbance at 254nm",suna_cols_flm$variable,fixed=TRUE)
suna_cols_flm$variable<-gsub("s275295z","Spectral Slope (275-295nm)",suna_cols_flm$variable,fixed=TRUE)

	ggplot(suna_cols_flm,aes(dtp,value))+geom_point(size=2)+
	...
	suna_cols_flm<-melt(subset(suna_cols_fl,date=="2015188"&hour(dtp)<12)[,c("dtp","a254","s275295z")],id.vars="dtp")
	suna_cols_flm$variable<-gsub("a254","Absorbance at 254nm",suna_cols_flm$variable,fixed=TRUE)
	suna_cols_flm$variable<-gsub("s275295z","Spectral Slope (275-295nm)",suna_cols_flm$variable,fixed=TRUE)
