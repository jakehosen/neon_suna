...
setwd("/Users/jhosen/gdrive/KING/2019")
...
suna_cal0<-read.csv("/Users/jhosen/gdrive/fdom/SUNA Calibration pdfs/suna_0839_20200312.csv")
suna_cal<-data.frame(Wavelength=seq(188,395,length.out=256),filler="fill",counts=approx(x=suna_cal0$Wavelength,y=suna_cal0$counts,xout=seq(188,395,length.out=256),rule=2)$y)
...
#wl188-395 from 12 to 267
#seq(188,395,length(seq(12,267)))
...
offse<-c(as.numeric(t(suna_cal_t[3,])))
...
compiled_suna2$n_253.7529<-as.numeric(compiled_suna2$n_253.7529)
...
compiled_suna2$n_350.3529<-as.numeric(compiled_suna2$n_350.3529)
compiled_suna2$sensor_temp<-as.numeric(compiled_suna2$sensor_temp)
compiled_suna2$date<-as.numeric(compiled_suna2$date)
compiled_suna2$int_time<-as.numeric(compiled_suna2$int_time)

summary(lm(a350~log10(n_350.3529)+I(log10(n_350.3529)^2)+I(log10(n_350.3529)^3),compiled_suna2))
summary(lm(a254~log10(n_253.7529)+I(log10(n_253.7529)^2)+I(log10(n_253.7529)^3),compiled_suna2))

a350_mod<-lm(a350~log10(n_350.3529)+I(log10(n_350.3529)^2)+I(log10(n_350.3529)^3),compiled_suna2)
a254_mod<-lm(a254~log10(n_253.7529)+I(log10(n_253.7529)^2)+I(log10(n_253.7529)^3),compiled_suna2)
...
for(i in 1:length(cor_cols)){
	prepdata<-data.frame(n_350.3529=compiled_suna2[,cor_cols[i]],n_253.7529=compiled_suna2[,cor_cols[i]])
...
for(i in 1:length(days)){
	...
	stop<-grep("_c350.mean$",names(ssp_temp))
	...
	ssp_rec$wl_nm<-gsub("_c350.mean","",gsub("n_","",ssp_rec$wl,fixed=TRUE),fixed=TRUE)
	ssp_rec_sum<-summaryBy(tslope~wl_nm,ssp_rec)
	



ss_plot3<-ss_plot2[,grep("c350.mean$",names(ss_plot2))]
ss_plot3$dtp_round<-ss_plot2$dtp_round
ssm<-melt(ss_plot3,id.vars=c("dtp_round"))
ssm$wl_nm<-as.numeric(gsub("_c350.mean","",gsub("n_","",ssm$variable,fixed=TRUE),fixed=TRUE))
...
ssm3$temp_cor<-ssm3$value+ssm3$value*((ssm3$sensor_temp.mean-20)*ssm3$tslope.mean)
...
scale_x_datetime(limits=c(as.POSIXct("2019-02-03"),as.POSIXct("2019-02-19")))+
geom_point()



#KING_PAR_Surf<-loadByProduct(dpID="DP1.20042.001",site="KING")
#saveRDS(KING_PAR_Surf,"~/gdrive/fdom/SUNA_NEON/KING/KING_PAR_Surf.rds")
KING_PAR_Surf<-readRDS("~/gdrive/fdom/SUNA_NEON/KING/KING_PAR_Surf.rds")
...
######Probably use this
pars<-as.data.frame(KING_PAR_Surf["PARWS_1min"])
...
for(i in 1:length(udates)){
	...
	mod_temp<-data.frame(date=udates[i],dark_ss=dark_ss,ss_light=light_extra,light_daily=light_daily,light_temp=light_temp,temp=temp,a254=mean(ss_temp$a254))
	light_results<-bind_rows(light_results,mod_temp)
}


ggplot(light_results,aes(light_daily,ss_light,color=(log10(a254))))+
geom_point(size=2)+
scale_y_log10()

summary(lm((ss_light)~light_daily+log10(a254),light_results))
...
lrs<-readRDS("~/gdrive/fdom/SUNA_NEON/SYCA/SYCA_light_results.rds")
lrs$site<-"SYCA"
lrc<-readRDS("~/gdrive/fdom/SUNA_NEON/CARI/CARI_light_results.rds")
lrc$site<-"CARI"
lrk<-readRDS("~/gdrive/fdom/SUNA_NEON/KING/KING_light_results.rds")
lrk$site<-"KING"



lrcc<-subset(bind_rows(lrs,lrc,lrk),a254>0 & a254<5)

KING_wq<-loadByProduct(dpID="DP1.20288.001",site="KING")
KING_wq_df<-as.data.frame(KING_wq[["waq_instantaneous"]])
KING_wq_df$date<-as.Date(KING_wq_df$startDateTime,tz="Etc/GMT+8")
KING_wq_sum<-summaryBy(turbidity~date,KING_wq_df,FUN=c(mean_na))
lrk2<-merge(lrk,KING_wq_sum,by="date",all.x=TRUE)
summary(lm(log10(ss_light)~light_daily+log10(a254)+turbidity.mean_na,lrk2))

ggplot(lrcc,aes(light_daily,ss_light,color=log10(a254)))+
geom_point(size=2)+
scale_y_log10()

mean_na<-function(x){mean(x,na.rm=TRUE)}
se <- function(x) sqrt(var(x,na.rm=TRUE)/length(!is.na(x)))
summs<-summaryBy(ss_light+light_daily+temp+a254~site,lrcc,FUN=c(mean_na,se))


summs$ss_light_min<-summs$ss_light.mean_na-summs$ss_light.se
summs$ss_light_max<-summs$ss_light.mean_na+summs$ss_light.se
summs$light_daily_min<-summs$light_daily.mean_na-summs$light_daily.se
summs$light_daily_max<-summs$light_daily.mean_na+summs$light_daily.se

neon_site_summary_light_ss275295<-ggplot(summs,aes(light_daily.mean_na,ss_light.mean_na))+
theme_ts_space+
geom_errorbar(aes(ymin=ss_light_min,ymax=ss_light_max),width=800)+
geom_errorbarh(aes(xmin=light_daily_min,xmax=light_daily_max),height=0.0006)+
geom_point(size=5)+
xlab(expression("Daily PAR ("*mu*moles*"-"*m^-2*s^-1*")"))+
ylab(expression("Sum of Daily "*SS[275-295]*" Deviation"))

pdf("~/gdrive/fdom/neon_site_summary_light_ss275295.pdf",width=6,height=6,useDingbats=FALSE)
neon_site_summary_light_ss275295
dev.off()

summary(lm((ss_light)~light_daily+log10(a254),lrcc))
	
library(ggfortify)
lrcc$log10_a254<-log10(lrcc$a254)
lrcc_pca_dat<-lrcc[complete.cases(lrcc[,c("ss_light","light_daily","log10_a254","temp")]),c("ss_light","light_daily","log10_a254","temp")]
lrcc_pca<-prcomp(lrcc_pca_dat,scale=TRUE)

lrcc_pca_dat$site<-lrcc[complete.cases(lrcc[,c("ss_light","light_daily","log10_a254")]),c("site")]
autoplot(lrcc_pca, data = lrcc_pca_dat, colour = 'site',loadings=TRUE, loadings.label=TRUE)	
	
	
	


cor_lm<-lm(a254~n_253.7529+I(n_253.7529^2)+sensor_temp,suna_d)
newdats<-data.frame(n_253.7529=suna_d$n_308.38,sensor_temp=suna_d$sensor_temp)
...
suna<-read.csv("/Users/jhosen/gdrive/KING/2019/D2019136.CSV",skip=14,header=FALSE)
#suna_d0<-cSplit(suna,3, ",")
...
summary(lm(a254~n_253.7529+I(n_253.7529^2)+sensor_temp,suna_d))
cor_lm<-lm(a254~n_253.7529+I(n_253.7529^2)+sensor_temp,suna_d)
newdats<-data.frame(n_253.7529=suna_d$n_308.38,sensor_temp=suna_d$sensor_temp)
suna_d$n_308.38_cor<-predict(cor_lm,newdata=newdats)
ggplot(suna_d,aes(dtp,a254,color=sensor_temp))+
