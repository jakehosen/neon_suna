...
setwd("/Users/jhosen/gdrive/fdom/SUNA_NEON/SYCA")
sunas<-list.files(pattern="*.csv")
sunas<-sunas[3]

#suna_cal<-readRDS("/Users/jhosen/gdrive/fdom/SUNA Calibration pdfs/suna_cal_comb.rds")
suna_cal<-read.csv("/Users/jhosen/gdrive/fdom/SUNA_NEON/SYCA/SUNA_681abbr.csv")
...
#doy<-gsub(".csv","",gsub("SYCA_2019","",paste(sunas[i]),fixed=TRUE),fixed=TRUE)
doy<-gsub("2019","",suna$date,fixed=TRUE)
...
compiled_suna2$n_253.72<-as.numeric(compiled_suna2$n_253.72)
compiled_suna2$a254<-as.numeric(compiled_suna2$a254)
compiled_suna2$sensor_temp<-as.numeric(compiled_suna2$sensor_temp)
summary(lm(a254~n_253.72+I(n_253.72^2)+I(n_253.72^3)+sensor_temp,compiled_suna2))

compiled_suna2$a350<-as.numeric(compiled_suna2$a350)
compiled_suna2$n_350.32<-as.numeric(compiled_suna2$n_350.32)
compiled_suna2$sensor_temp<-as.numeric(compiled_suna2$sensor_temp)
compiled_suna2$date<-as.numeric(compiled_suna2$date)
compiled_suna2$int_time<-as.numeric(compiled_suna2$int_time)

summary(lm(a350~log10(n_350.32)+I(log10(n_350.32)^2)+I(log10(n_350.32)^3),compiled_suna2))
summary(lm(a254~log10(n_253.72)+I(log10(n_253.72)^2)+I(log10(n_253.72)^3),compiled_suna2))

a350_mod<-lm(a350~log10(n_350.32)+I(log10(n_350.32)^2)+I(log10(n_350.32)^3),compiled_suna2)
a254_mod<-lm(a254~log10(n_253.72)+I(log10(n_253.72)^2)+I(log10(n_253.72)^3),compiled_suna2)
...
for(i in 1:length(cor_cols)){
	prepdata<-data.frame(n_350.32=compiled_suna2[,cor_cols[i]],n_253.72=compiled_suna2[,cor_cols[i]])
...
ssm3$temp_cor<-ssm3$value+ssm3$value*((ssm3$sensor_temp.mean-25)*ssm3$tslope.mean)
...
#SYCA_PAR_Surf<-loadByProduct(dpID="DP1.20042.001",site="SYCA")
#saveRDS(SYCA_PAR_Surf,"~/gdrive/fdom/SUNA_NEON/SYCA/SYCA_PAR_Surf.rds")
SYCA_PAR_Surf<-readRDS("~/gdrive/fdom/SUNA_NEON/SYCA/SYCA_PAR_Surf.rds")
pars<-as.data.frame(SYCA_PAR_Surf["PARWS_1min"])
...
pars_ss_m<-melt(pars_ss_temp[,c("dtp","s275295","spec_temp.mean","PARWS_1min.PARMean")],id=c("dtp"))

ggplot(pars_ss_m,aes(dtp,value,group=variable))+
geom_point()+
facet_wrap(.~variable,scale="free_y")

pdf("~/gdrive/fdom/diel_suna.pdf", width=5, height=8, useDingbats=FALSE)
pars_ss_m2<-melt(pars_ss_temp[,c("dtp","s275295","spec_temp.mean","PARWS_1min.PARMean.mean_na")],id=c("dtp"))
ggplot(subset(pars_ss_m2,as.Date(dtp,tz="Etc/GMT+8")=="2019-06-01"),aes(dtp,value,group=variable))+
theme_ts_space+
...
geom_point()+
xlab("")+
facet_wrap(.~variable,scale="free_y",ncol=1,strip.position=c("left"))
dev.off()
...
ggplot(subset(pars_ss_temp,as.Date(dtp,tz="Etc/GMT+8")=="2019-07-25"),aes(dtp,s275295))+
geom_point()


	
cs2_uvvis<-compiled_suna2[,grep("_c254$",names(compiled_suna2))]
cs2_uvvis$dtp<-compiled_suna2$dtp
#cs2_uvvis_2<-subset(cs2_uvvis,dtp>=as.POSIXct("2019-06-01") & dtp<=("2019-06-30"))
cs2_uvm<-melt(cs2_uvvis,id=c("dtp"))
cs2_uvm$wl_nm<-gsub("n_","",gsub("_c254","",cs2_uvm$variable,fixed=TRUE),fixed=TRUE)	
cs2_uvm2<-subset(cs2_uvm,wl_nm>225)	
cs2_uvm2$wl_nm<-as.numeric(cs2_uvm2$wl_nm)
cs2_uvm3<-summaryBy(value~variable+wl_nm+dtp,cs2_uvm2)
abs_spectra_time<-ggplot(cs2_uvm3,aes(wl_nm,value.mean,color=dtp))+
theme_ts_space+
theme(legend.position="right")+
scale_color_viridis(name="Date & Time",discrete=FALSE)+
geom_point(size=2)+
xlab("Wavelength (nm)")+
ylab(expression("Absorbance ("*m^-1*")"))

pdf("~/gdrive/fdom/abs_spectra_time.pdf",width=10,height=10,useDingbats=FALSE)
abs_spectra_time
dev.off()
...
cor_lm<-lm(a254~n_253.72+I(n_253.72^2)+sensor_temp,suna_d)
newdats<-data.frame(n_253.72=suna_d$n_308.38,sensor_temp=suna_d$sensor_temp)
...
summary(lm(a254~n_253.72+I(n_253.72^2)+sensor_temp,suna_d))
cor_lm<-lm(a254~n_253.72+I(n_253.72^2)+sensor_temp,suna_d)
newdats<-data.frame(n_253.72=suna_d$n_308.38,sensor_temp=suna_d$sensor_temp)
suna_d$n_308.38_cor<-predict(cor_lm,newdata=newdats)
ggplot(suna_d,aes(dtp,a254,color=sensor_temp))+
